namespace SerialportCli
{
    ...
    internal static class EchoCommand
    {
        ...
        public static Command Build()
        {
            ...
            command.Handler = CommandHandler.Create<InvocationContext, GlobalParams, EchoParams>(Run);
            return command;
        }

        private static async Task<int> Run(InvocationContext context, GlobalParams globalParams, EchoParams @params)
        {
            ...
            context.GetCancellationToken().Register(() => waiter.TrySetResult(0));

            var initBytes = context.ParseResult.ValueForOption<int?>("--init-bytes");
